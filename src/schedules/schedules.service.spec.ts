...
describe('SchedulesService', () => {
  ...
  beforeEach(async () => {
    ...
    repository.findSchedulesInDateRange.mockResolvedValue([]);
    ...
    workerUnavailabilityService.getUnavailableWorkerIds.mockResolvedValue(new Map());
  ...
  it('returns worker and blocking schedule details for a date conflict', async () => {
    const blockingScheduleId = new Types.ObjectId();
    repository.findWorkerConflictOnDate.mockResolvedValue({
      _id: blockingScheduleId,
      service_type: ServiceType.Youth,
      assignments: [{
        worker_id: new Types.ObjectId(leaderId),
        worker_name: 'Leader',
    ...
    try {
      ...
      throw new Error('Expected schedule creation to fail');
    } catch (error: any) {
      expect(error.getResponse()).toMatchObject({
        errors: {
          ...
          conflicts: [expect.objectContaining({
            ...
            worker_name: 'Leader',
            blocking_schedule: {
              id: blockingScheduleId.toString(),
              service_type: ServiceType.Youth,
              role: WorkerRole.Leader,
              slot_key: 'leader',
            },
  ...
  it('rejects incomplete generated schedules during confirm', async () => {
    const result = await service.confirmAutoGeneratedSchedules({
        schedules: [
          {
            date: '2099-07-12',
            assignments: [
    ...
    expect(result.items[0].conflicts).toHaveLength(2);
  });
  ...
  it('returns assignment constraints with schedule and unavailability context', async () => {
    repository.findSchedulesInDateRange.mockResolvedValue([{
      _id: new Types.ObjectId('66d000000000000000000001'),
      date: new Date('2099-07-12T00:00:00.000Z'),
      service_type: ServiceType.Youth,
      assignments: [{
        worker_id: new Types.ObjectId(leaderId),
    ...
    workerUnavailabilityService.getUnavailableWorkerIds.mockResolvedValue(
      new Map([['2099-07-19', new Set([drumsId])]]),
    );

    const result = await service.getAssignmentConstraints('2099', '7');

    expect(result.dates).toEqual(expect.arrayContaining([
      expect.objectContaining({
        date: '2099-07-12',
        blocked_workers: [expect.objectContaining({
          worker_id: leaderId,
          code: 'WORKER_ALREADY_ASSIGNED',
        })],
      }),
  ...
  it('partially confirms generated schedules and reports the failed date', async () => {
    const result = await service.confirmAutoGeneratedSchedules({
      schedules: [
        {
          date: '2099-07-12',
    ...
    expect(result.summary).toEqual({ submitted: 2, created: 1, failed: 1 });
    expect(result.items.map((item) => item.status)).toEqual(['created', 'failed']);
    expect(repository.insertRecord).toHaveBeenCalledTimes(1);
  });
